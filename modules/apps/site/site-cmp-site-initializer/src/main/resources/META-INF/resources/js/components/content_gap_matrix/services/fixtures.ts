...
/**
 * Mock fixtures for the Content Coverage Matrix.
 *
 * Terms match the seeded values from LPD-94098 (Personas) and LPD-94092
 * (Funnel Stage), in vocabulary order (never alphabetical). The external
 * reference codes match the categories created in the Global site for dev, so
 * the identifiers line up when the mock is swapped for the real service.
 */
...
/**
 * Partial coverage with gaps. The realistic default. Cell sum (78) exceeds
 * totalAssetCount (42) because an asset can carry several personas.
 */
...
/**
 * Assets exist but none are categorized. Drives the "Start Mapping" prompt.
 */
...
/**
 * No assets at all. Drives the "No Assets Yet" empty state.
 */
