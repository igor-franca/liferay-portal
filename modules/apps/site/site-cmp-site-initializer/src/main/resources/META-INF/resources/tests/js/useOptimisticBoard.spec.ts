...
describe('useOptimisticBoard', () => {
	it('initialize with the correct board data', () => {
	...
	it('optimistically update the board when a task is moved', async () => {
		const onTaskMoveApi = jest.fn(
			() =>
				new Promise<RequestResult<ITask>>((resolve) =>
					setTimeout(
						() => resolve({error: null} as RequestResult<ITask>),
						100
					)
				)
		);
	...
	it('revert the optimistic update if the API call fails', async () => {
		const onTaskMoveApi = jest.fn(() =>
			Promise.resolve({error: 'API Error'} as RequestResult<ITask>)
		);
	...
	it('update the server state when the API call is successful', async () => {
		const onTaskMoveApi = jest.fn(() =>
			Promise.resolve({error: null} as RequestResult<ITask>)
		);
