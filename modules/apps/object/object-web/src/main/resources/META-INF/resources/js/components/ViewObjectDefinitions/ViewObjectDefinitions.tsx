...
export default function ViewObjectDefinitions({
	...
	const initialValues: ObjectFolderRequestInfo = {
		actions: {
			delete: emptyAction,
			get: emptyAction,
			permissions: emptyAction,
			update: emptyAction,
			updateBatch: emptyAction,
		},
		items: [],
	};
		...
		if (Liferay.FeatureFlags['LPS-148856']) {
			const makeFetch = async () => {
				API.getAllObjectFolders().then((response) => {
					setObjectFolderRequestInfo(response);
		...
		<>
							...
							<Card
								...
								customHeader={
									<ObjectFolderCardHeader
										...
										items={
											getObjectFolderActions({
												actions: selectedObjectFolder.actions,
												id: selectedObjectFolder.id ?? 0,
												objectFolderPermissionsURL,
												setShowModal,
											}) as IItem[]
										}
			...
			{showModal.addObjectFolder && (
				<ModalAddObjectFolder
					...
					setObjectFolderRequestInfo={setObjectFolderRequestInfo}
