...
export function ObjectRelationshipFormBase({
	...
	useEffect(() => {
		const fetchObjectDefinition = async () => {
			const object1 = await API.getObjectDefinitionByExternalReferenceCode(
				objectDefinitionExternalReferenceCode1 as string
			);
			let object2 = null;

			if (objectDefinitionExternalReferenceCode2) {
				object2 = await API.getObjectDefinitionByExternalReferenceCode(
					objectDefinitionExternalReferenceCode2 as string
				);

				setObjectDefinition2(object2);
			}
			setCurrentObjectDefinition(object1);
			setCreationLanguageId(object1.defaultLanguageId);
			setObjectDefinition1(object1);

			setValues({
				objectDefinitionExternalReferenceCode1:
					object1.externalReferenceCode,

				objectDefinitionExternalReferenceCode2:
					object2?.externalReferenceCode,
				objectDefinitionId1: object1.id,
				objectDefinitionId2: object2?.id,
			});

			handleObjectRelationshipTypes(object1);
		};
