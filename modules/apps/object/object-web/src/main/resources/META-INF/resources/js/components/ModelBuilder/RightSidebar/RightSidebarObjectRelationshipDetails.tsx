...
export function RightSidebarObjectRelationshipDetails({
	...
	const [{baseResourceURL, elements}, dispatch] = useFolderContext();
	...
	const [restContextPath, setRestContextPath] = useState('');
	...
	const onSubmit = async () => {
		...
		if (!Object.keys(validationErrors).length) {
			const relationship = {
				...values,
				objectRelationshipId: values.id,
			};

			try {
				await API.updateRelationship(relationship);
			...
			const updatedElements = elements.map((element) => {
				if (
					...
					(element as Edge<ObjectRelationshipEdgeData>).data
						...
						relationship?.objectRelationshipId
				) {
	...
	return (
		<>
			...
			<div className="lfr-objects__model-builder-right-sidebar-relationship-content">
				...
				{parameterRequired && selectedEdge.data?.type === 'oneToMany' && (
					<>
						<Input
