			...
			<NewAppPageFooterButtons
				...
				onClickContinue={async () => {
					...
					if(supportURLSpecificationId != -1){
						dispatch({
							payload: { id: supportURLSpecificationId, value: supportURL.value },
							type: TYPES.UPDATE_APP_SUPPORT_URL,
						  })
					  }
					  else{
						dispatch({
							payload: { id: supportURL?.id, value: supportURL.value },
							type: TYPES.UPDATE_APP_SUPPORT_URL,
						  })
					  }
					...
					if (publisherWebsiteURL?.value) {
						...
						if(publisherWebsiteURLSpecificationId != -1){
							dispatch({
								payload: { id: publisherWebsiteURLSpecificationId, value: publisherWebsiteURL.value },
								type: TYPES.UPDATE_APP_PUBLISHER_WEBSITE_URL,
							  })
						  }
						  else{
							dispatch({
								payload: { id: publisherWebsiteURL?.id, value: publisherWebsiteURL.value },
								type: TYPES.UPDATE_APP_PUBLISHER_WEBSITE_URL,
							  })
						  }
					}
					if (appUsageTermsURL?.value) {
						...
						if(appUsageTermsURLSpecificationId != -1){
							dispatch({
								payload: { id: appUsageTermsURLSpecificationId, value: appUsageTermsURL.value },
								type: TYPES.UPDATE_APP_USAGE_TERMS_URL,
							  })
						  }
						  else{
							dispatch({
								payload: { id: appUsageTermsURL?.id, value: appUsageTermsURL.value },
								type: TYPES.UPDATE_APP_USAGE_TERMS_URL,
							  })
						  }
					}
					if (appDocumentationURL?.value) {
						...
						if(appDocumentationURLSpecificationId != -1){
							dispatch({
								payload: { id: appDocumentationURLSpecificationId, value: appDocumentationURL.value },
								type: TYPES.UPDATE_APP_DOCUMENTATION_URL,
							  })
						  }
						  else{
							dispatch({
								payload: { id: appDocumentationURL?.id, value: appDocumentationURL.value },
								type: TYPES.UPDATE_APP_DOCUMENTATION_URL,
							  })
						  }
					}
					if (appInstallationGuideURL?.value) {
						...
						if(appInstallationGuideURLSpecificationId != -1){
							dispatch({
								payload: { id: appInstallationGuideURLSpecificationId, value: appInstallationGuideURL.value },
								type: TYPES.UPDATE_APP_INSTALLATION_AND_UNINSTALLATION_GUIDE_URL,
							  })
						  }
						  else{
							dispatch({
								payload: { id: appInstallationGuideURL?.id, value: appInstallationGuideURL.value },
								type: TYPES.UPDATE_APP_INSTALLATION_AND_UNINSTALLATION_GUIDE_URL,
							  })
						  }
					}
